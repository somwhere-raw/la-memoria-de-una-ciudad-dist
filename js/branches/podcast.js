...
function seekToTranscriptLine(lineEl) {
  ...
  const knownDuration = Number.isFinite(audioEl.duration) && audioEl.duration > 0
    ? audioEl.duration
  ...
  // Solicitar reproducción dentro del gesto del usuario evita bloqueos de autoplay.
  try {
  ...
  if (liveStatusEl) {
    const selectedText = lineEl.querySelector('.transcript-line__text')?.textContent || '';
